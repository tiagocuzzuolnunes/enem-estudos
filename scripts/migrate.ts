/**
 * Safe migration — never deletes or overwrites user data.
 *
 * What it does:
 *   - Upserts areas, subjects, subareas, topics, subtopics by their natural key
 *   - For NEW subtopics: sets defaults (priority, completed, empty links)
 *   - For EXISTING subtopics: only updates structural fields (order, parent refs)
 *     — completed, priority, videoLinks, exerciseLinks, additionalLinks are untouched
 *   - Upserts Settings without overwriting existing enemDate
 *
...
const SubtopicSchema = new mongoose.Schema({
  ...
  exerciseLinks: { type: Array, default: [] },
...
async function migrate() {
  ...
  for (let ai = 0; ai < AREAS.length; ai++) {
    ...
    const area = await Area.findOneAndUpdate(
      { slug: areaData.slug },
      { $set: { name: areaData.name, color: areaData.color, icon: areaData.icon, order: areaData.order } },
    ...
    for (let si = 0; si < areaContent.subjects.length; si++) {
      ...
      const subject = await Subject.findOneAndUpdate(
        { areaId: area._id, name: subjectData.name },
        { $set: { order: si } },
        { upsert: true, new: true }
      )
      ...
      for (let sai = 0; sai < subjectData.subareas.length; sai++) {
        ...
        const subarea = await Subarea.findOneAndUpdate(
          { subjectId: subject._id, name: subareaData.name },
          { $set: { areaId: area._id, order: sai } },
          { upsert: true, new: true }
        )
        ...
        for (let ti = 0; ti < subareaData.topics.length; ti++) {
          ...
          const topic = await Topic.findOneAndUpdate(
            { subareaId: subarea._id, name: topicData.name },
            { $set: { subjectId: subject._id, areaId: area._id, order: ti } },
            { upsert: true, new: true }
          )

          for (let sti = 0; sti < topicData.subtopics.length; sti++) {
            const name = topicData.subtopics[sti]

            const existing = await Subtopic.findOne({ topicId: topic._id, name })

            if (existing) {
              // Only update structural fields — never touch user data
              await Subtopic.updateOne(
                { _id: existing._id },
                { $set: { order: sti, subareaId: subarea._id, subjectId: subject._id, areaId: area._id } }
              )
              skipped++
            } else {
              await Subtopic.create({
                ...
                videoLinks: [], exerciseLinks: [], additionalLinks: [],
              })
              created++
            }
          }
  ...
  // Upsert settings without overwriting existing enemDate
  ...
  console.log(`\nDone: ${created} subtopics created, ${skipped} existing preserved.`)
